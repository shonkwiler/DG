...
\begin{example}\label{ex:affine geodesics}
	Let's figure out the geodesic equation on $\Aff^+(\R) \cong H = \{(x,y) \in \R^2 : y > 0\}$. We saw in \Cref{ex:affine hyperbolic metric} that the metric is given by $g_{11} = \frac{1}{y^2} = g_{22}$, $g_{12} = 0$, and we worked out the Levi-Civita connection in \Cref{ex:hyperbolic plane connection}. Specifically,~\eqref{eq:hyperbolic plane Christoffel} implies that
	\begin{align*}
